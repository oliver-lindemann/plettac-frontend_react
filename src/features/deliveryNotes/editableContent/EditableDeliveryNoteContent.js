...
import {
  DELIVERY_NOTE_LOGISTICS,
  DELIVERY_NOTE_TYPE,
} from "../../../config/deliveryNote";
import { LIST_STATUS, LIST_STATUS_LANG } from "../../../config/list";

import { dayjsUTC, getDayjsWithoutTime } from "../../../utils/DateUtils";
import {
  copyArray,
  sortChecklistPartsByOrderIndex,
} from "../../../utils/checklist/ChecklistPartsUtils";
...
const EditableDeliveryNoteContent = ({ deliveryNote, onSaveButtonClicked }) => {
  ...
  const {
    ...
    defaultValue: "",
    initialValue:
      deliveryNote?.warehouseWorker?._id ||
      deliveryNote?.warehouseWorker ||
      user?.id,
  ...
  const [dateOfIssue, setDateOfIssue] = useState(
    deliveryNote?.dateOfIssue ? dayjsUTC(deliveryNote.dateOfIssue) : dayjsUTC()
  );
  ...
  const [visible, setVisible] = useState(
    deliveryNote?._id ? deliveryNote?.visible : user?.isAdmin ? false : true
  );
  const [shouldBeLoaded, setShouldBeLoaded] = useState(
    user?.isAdmin ? true : false
  );
  const [deliveryNoteParts, setDeliveryNoteParts] = useState(
    sortChecklistPartsByOrderIndex(copyArray(deliveryNote?.parts)) || []
  );
  const [deliveryNoteCustomParts, setDeliveryNoteCustomParts] = useState(
    deliveryNote?.customParts || []
  );
  ...
  useEffect(() => {
    ...
    if (isDirty) {
      ...
      localStorage.setItem(
        deliveryNote?._id || DELIVERY_NOTE_DRAFT,
        JSON.stringify(deliveryNoteDraft)
      );
    }
    ...
    note,
    deliveryNoteParts,
    deliveryNoteCustomParts,
  ]);

  useEffect(() => {
    if (createdCustomer) {
      const foundCustomer = customers.find(
        (customer) => customer.name === createdCustomer.name
      );
      if (foundCustomer) {
  ...
  const handleDeliveryNoteFileImported = (updatedDeliveryNote) => {
    ...
    setCustomer(
      customers?.find(
        (customer) => customer.name === updatedDeliveryNote.customer
  ...
  const onCustomerCreated = (createdCustomer) =>
  ...
  const handleSaveDeliveryNote = async (e) => {
    ...
    if (!shouldBeLoaded) {
      ...
      updatedDeliveryNote.customParts = updatedDeliveryNote.customParts.map(
        (partItem) => ({
          ...partItem,
          loaded: partItem.amount,
        })
    ...
    <Grid item xs={12} sm={12} md={12} lg={5} xl={4}>
      ...
      {deliveryNote?._id && (
        <TextField
          ...
          helperText={
            ...
            "Bitte gib eine gültige Lieferscheinnummer (> 0) ein."
          }
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">LS-</InputAdornment>
            ),
            endAdornment:
              !!uniqueNumberIsTouched &&
              (!!uniqueNumberHasError ? (
            ...
            <ItemSelect
              ...
              renderListItem={(props, option) => (
                <div
                  ...
                  className="px-2 py-1 virtualizedListItem"
                >
                  <Typography fontWeight="bold">{option.name}</Typography>
                  <Typography color="#666">
                    {option.street || "<keine Straße>"},{" "}
                    {option.city || "<keine Stadt>"}
                  </Typography>
                </div>
              )}
              renderInput={(params) => (
                <TextField
                  ...
                  helperText={
                    !!customerHasError && "Bitte wähle einen Kunden aus."
                  }
                  InputProps={{
                    ...params.InputProps,
                    startAdornment: (
      ...
      <TextField
        ...
        helperText={
          ...
          "Bitte gib die anwesende Person des Kunden ein."
        }
        InputProps={{
          startAdornment: (
            <InputAdornment position="start">
      ...
      {!!users ? (
        <TextField
          ...
          helperText={
            ...
            "Bitte wähle den zuständigen Lagermitarbeiter aus."
          }
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
        ...
        >
          {users.map((user) => (
            <MenuItem key={user._id} className="mr-3" value={user._id}>
              <ListItemAvatar>
      ...
      <div className="mt-3">
        ...
        >
          <ToggleButton
            value={DELIVERY_NOTE_LOGISTICS.OUTBOUND}
            color="primary"
          >
            <FileUploadOutlined />
            <div className="mx-1" /> Ausgabe
          </ToggleButton>
          <ToggleButton
            value={DELIVERY_NOTE_LOGISTICS.INBOUND}
            color="secondary"
          >
            <FileDownloadOutlined />
            <div className="mx-1" /> Rücklieferung
          </ToggleButton>
        </ToggleButtonGroup>
      </div>
      ...
      <TextField
        ...
        InputProps={{
          ...
          endAdornment: !noteHasError && noteIsTouched && (
            <InputAdornment position="end">
              <CheckCircleOutlined color="success" />
            </InputAdornment>
          ),
      ...
      {deliveryNote?._id && (
        <TextField
          ...
          helperText={
            !!statusHasError && "Bitte wähle den aktuellen Status aus."
          }
          InputProps={{
            startAdornment: (
              <InputAdornment position="start">
            ...
            endAdornment: !statusHasError && statusIsTouched && (
              <InputAdornment position="end">
                <CheckCircleOutlined color="success" />
              </InputAdornment>
            ),
      ...
      {user?.isAdmin ? (
        <div className="ms-1 mt-3">
          <FormControlLabel
            ...
            label="Dieser Lieferschein muss noch verladen werden"
            sx={{ width: "100%" }}
          />
          <div className="mt-3">
            <Button
            ...
            <Typography variant="subtitle1">
              Wenn die Liste sichtbar ist (grünes Auge), kann diese von
              Lagermitarbeitern angesehen werden. Wenn die Liste verborgen ist
              (rotes Auge), kann das Lager diese Liste nicht einsehen.
            </Typography>
          </div>
        </div>
      ) : null}
